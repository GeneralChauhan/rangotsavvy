...
Add to `.env.local` (local) and to your **production environment variables** (e.g. Vercel → Project → Settings → Environment Variables) so ticket emails work in prod:
...
If these are missing in production, the API returns `"Email service is not configured."` and the booking still succeeds but no ticket email is sent.
